...
void UGHMoveComponent::StartCharacterMove(int32 DistanceInHexMaxMove)
{
	if(SpawnHexActor)
	{
	...
	ShowMovableHex(DistanceInHexMaxMove);
}

void UGHMoveComponent::ShowMovableHex(int32 MaxHex)
{
	TArray<AGHHexActor*> MovableHex;
	for(int32 i=0; i<MaxHex; i++)
	{
		if(i==0)
		{
			MovableHex = ActorLocationHex->FriendsHexArr;
		}
		TArray<AGHHexActor*> Cach;
		for(auto hex: MovableHex)
		{
			if(MovableHex.Find(hex)==INDEX_NONE)
			{
				//Cach.Add(hex->FriendsHexArr);
			}
		}
	}
}
...
//Call in tick to move character on the path

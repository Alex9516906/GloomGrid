...
AGHHexActor* AGHSpawnGrid::GetHexFromCoord(int32 X, int32 Y)
{
	for(auto hexActor:HexArray)
	{
		if(hexActor->CoordX == X && hexActor->CoordY==Y)
		{
			return hexActor;
		}
	}
...
FVector2D AGHSpawnGrid::GetPointHex(const float Size, FVector Centre, int32 I)
{
